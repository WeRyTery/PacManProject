    ...
    def draw(self, window):
        ...
        if not self.scared:
            pg.draw.circle(
                window,
                WHITE,
                (int(self.x - eye_offset), int(self.y - 2)),
                eye_radius + 1,
            )
            pg.draw.circle(
                window,
            ...
            pg.draw.circle(
                window,
                BLACK,
                (int(self.x - eye_offset + px * 2), int(self.y - 2 + py * 2)),
                pupil_radius,
            )
            pg.draw.circle(
                window,
        ...
        if self.state == "EXITING":
            if abs(self.x - self.exit_x) > self.speed:
                ...
                if self.y <= self.target_y:
                    ...
                    self.state = "ALIVE"
                    self.vel_x, self.vel_y = random.choice(
            ...
            if self.is_at_center():
                ...
                for d in dirs:
                    ...
                    if self.can_move(d[0], d[1]) and not (
                        d[0] * self.vel_x < 0 or d[1] * self.vel_y < 0
                    ):
                        valid.append(d)

                if valid:
                    if self.scared:
                        self.vel_x, self.vel_y = max(
                            valid, key=lambda d: self.get_dist(d, target_pos)
                        )
                    else:
                        self.vel_x, self.vel_y = min(
                            valid, key=lambda d: self.get_dist(d, target_pos)
                        )
