...
class GhostHandler:
    ...
    def update(self, pacman):
        ...
        self.temp_scores = [
            s for s in self.temp_scores if current_time - s["time"] < 1000
        ]
        ...
        for ghost in self.ghosts:
            ...
            if not ghost.scared and ghost.state == "ALIVE":
                if ghost.normal_color == PINK:
                    target = (
                        pacman.x + pacman.vel_x * 4 * SQUARE_SIZE,
                        pacman.y + pacman.vel_y * 4 * SQUARE_SIZE,
                    )
                elif ghost.normal_color == ORANGE:
                    dist = (
                        (ghost.x - pacman.x) ** 2 + (ghost.y - pacman.y) ** 2
                    ) ** 0.5
                    if dist < 8 * SQUARE_SIZE:
                        target = (CENTERING_W, CENTERING_H + 28 * SQUARE_SIZE)
    ...
    def reset_ghost(self, ghost):
        ...
        ghost.x, ghost.y = (
            CENTERING_W + 13 * SQUARE_SIZE + SQUARE_SIZE // 2,
            CENTERING_H + 14 * SQUARE_SIZE + SQUARE_SIZE // 2,
        )
